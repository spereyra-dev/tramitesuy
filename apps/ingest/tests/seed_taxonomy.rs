...
//! scratch database. The real `data/` seed (34 events over 13 categories,
...
async fn seed_taxonomy_writes_every_projection_and_is_idempotent() {
    ...
    assert_eq!(
        category_count, 13,
        "all thirteen taxonomy categories must be seeded (Vehículos + the twelve expansion categories)"
    );
    assert_eq!(
        event_count, 34,
        "all thirty-four taxonomy events must be seeded (TX-5, including the consumer event)"
    );
    assert!(keyword_count > 0, "typed keywords must be projected");
    assert_eq!(
        synonym_count, 27,
        "the synonyms projection must match the YAML seed (lote-1: 27 total synonyms)"
    );
    assert!(
        relation_count > 0,
