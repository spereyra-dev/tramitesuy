...
//! scratch database. The real `data/` seed (104 events over 14 categories,
...
async fn seed_taxonomy_writes_every_projection_and_is_idempotent() {
    ...
    assert_eq!(
        event_count, 104,
        "all one hundred and four taxonomy events must be seeded (TX-5, including the consumer event)"
    );
