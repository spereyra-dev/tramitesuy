...
- Work-unit commits: T12 `683a207 feat(db): add catalog generation manifest migration`; T13 `0b68e88 feat(db): add ingestion run migration`; T14 `aa94eb3 feat(db): add generation projection migrations`.
...
- [x] **T14 — Projection migration**: Add `0015_generation_projections.sql` and migration coverage for per-generation projection tables, unique keys, and trigram index.
  ...
  - Evidence: strict TDD RED observed (five projection tables absent), then GREEN independently verified. All tables are generation-scoped, use unique `(generation_id, slug)`, and the trigram index targets `generation_trigram_surface.surface_text` with `gin_trgm_ops`. Work-unit commit: `aa94eb3 feat(db): add generation projection migrations`.
